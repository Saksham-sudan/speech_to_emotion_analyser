...
@st.cache_resource
def load_models():
    with st.spinner("Loading AI Models (DistilHuBERT + Whisper)..."):
        # Emotion (Custom Model)
        MODEL_PATH = "./distilhubert_hybrid_final"
        try:
            processor = AutoFeatureExtractor.from_pretrained(MODEL_PATH)
            model = AutoModelForAudioClassification.from_pretrained(MODEL_PATH)
        except Exception as e:
            st.error(f"Error loading custom model from {MODEL_PATH}: {e}")
            st.stop()
